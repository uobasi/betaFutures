...
def update_graph_live(n_intervals, sname, interv, stored_data, previous_stkName, previous_interv, interval_time): #interv
    ...
    try:
        ...
        df['slope_degrees'] = [calculate_slope_rolling(i, df['smoothed_1ema'].values, int(12)) for i in range(len(df))]
        df['polyfit_slope'] = [calculate_polyfit_slope_rolling(i, df['smoothed_1ema'].values, int(12)) for i in range(len(df))]
