    ...
    if len(clusterList) > 0:
        ...
        for cluster in clusterList:
            if cluster[1] > mazz:
                ...
                if (abs(float(maxNum) - df['1ema'][len(df)-1]) / ((float(maxNum) + df['1ema'][len(df)-1]) / 2)) * 100 <= 1 or (abs(float(minNum) - df['1ema'][len(df)-1]) / ((float(minNum) + df['1ema'][len(df)-1]) / 2)) * 100 <= 1 : #0.25
                    fig.add_shape(
                        type="rect",
